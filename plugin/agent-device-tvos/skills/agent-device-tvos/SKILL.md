...
tvOSはSiri Remoteのフォーカスエンジンで駆動する。座標タップ・タッチジェスチャは存在しない。デバイス指定は `--platform ios --target tv`。実機Apple TVとシミュレータが同時に見えることがあるため、`--device <name>` を常に明示する。シミュレータは署名envなしで動くが、実機はランナー署名のセットアップが必要（references/runner-signing.md）。
...
- `snapshot -i` などAXツリー系（get / is / --overlay-refs / diff snapshot）の生死は環境と画面で割れる: simでは不安定（実測: 失敗が頻繁だが、同一画面で成功に転じることもある）、実機は静的画面なら安定して成功する（実測: リスト・ルート画面含め8/8、1回7〜9秒）が、動画マストヘッド等のアニメーション中は同じ「overwhelming」失敗になる（実測）。コアループはどちらでも成立するようAXに依存させず、テキストの確認はAX死でも動く表面読みの `wait text` / `find "<text>" exists` を使う（waits-and-values.md）。
...
以下のトリガーに合致する時だけリファレンスを読む。先読みや全部読みはしない。表には執筆済みのリファレンスのみ載せる。
...
references/runner-signing.md	実機で最初のUIコマンドが「xcodebuild build-for-testing failed」で失敗するとき、またはrunner.logに「No Account for Team」「No profiles ... were found」「Your team has no devices」が出るとき
references/daemon-lifecycle.md	署名envを前置しても既定チーム（No Account for Team "2S799L9W4M"）に戻るとき、またはdaemonの生死・envの継承を確認するとき
references/device-trust.md	実機がunavailable・ペアリング不明のとき、または「device is still connecting」「unlocked, trusted, and available in Xcode > Devices」のHintが出るとき
