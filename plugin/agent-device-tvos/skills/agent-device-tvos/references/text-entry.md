...
- `fill '<selector>' "<text>"` はtvOSでは実用にならない。simではセレクタ解決（AXツリー）がsnapshotと同じ「overwhelming」エラーで失敗し（実測）、AXが生きている実機でも実在の検索フィールドに対して `Error (COMMAND_FAILED): Ref @eN is off-screen and not safe to fill` で拒否された（device実測）。入力は フォーカス→type の経路に統一する。
