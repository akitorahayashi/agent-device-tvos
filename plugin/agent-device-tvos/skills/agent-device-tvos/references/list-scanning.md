...
1. 目標の存在確認: `wait text "<目標>"`（部分一致・bare）。表面読みは画面外の項目にもマッチする——SwiftUIの遅延生成リストでも全行が見える（sim実測: 100行リストの末尾行に未スクロールでヒット）。つまり wait text は「存在確認」にしか使えず、到達（フォーカスが目標行に乗った）の判定には使えない。成功すれば走査する価値がある。
...
- `scroll down` はスクロール可能なリストではフォーカスを1行動かす（down押下と同等の変化をdiffで実測）。スクロール不能な画面では成功報告のまま何も起きない（sim実測）。実機ではアニメーション中の画面で `Error (COMMAND_FAILED): main thread execution timed out` で失敗した（device実測）——press downの下位互換であり、走査はtv-remoteに統一する。
