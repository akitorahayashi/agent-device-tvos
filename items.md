...
| list-scan | 画面外の目標へ有限ループで走査し端到達を判定できる | wait text → tv-remote press down → diff screenshot の反復 | sim+device | auto | 0.19.3（fixtureの長リストでpress down→differ を実測） | tests/sim/remote.test.ts | list-scanning.md | 端到達はdiff=matchで判定 |
